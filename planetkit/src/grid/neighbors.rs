...
impl Neighbors {
    pub fn new(pos: GridPoint3, resolution: [GridCoord; 2]) -> Neighbors {
        ...
        if is_away_from_root_edges {
            ...
            Neighbors {
                iter: NeighborsImpl::SlowGeneral(
                    above_and_below.chain(slow_general_edge_neighbors),
                ),
            }
        }
    }
...
impl Iterator for SlowGeneralEdgeNeighbors {
    ...
    fn next(&mut self) -> Option<GridPoint3> {
        ...
        turn_left_by_one_hex_edge(&mut self.origin, &mut self.current_dir, self.resolution)
            .expect("Oops, we picked a bad starting direction.");
...
impl Iterator for AboveAndBelow {
    ...
    fn next(&mut self) -> Option<GridPoint3> {
        if !self.yielded_above {
            ...
            return Some(self.origin.with_z(self.origin.z + 1));
        }
